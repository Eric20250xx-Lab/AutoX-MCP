...
- “启用脚本守护”：默认关闭。
- “守护脚本路径”：默认为空；填写相对于 AutoX 当前脚本目录的路径，
  例如 `icbc_attendance_command_agent.js`。绝对路径、包含 `..` 的路径，以及
  canonical 解析后越出当前脚本目录的路径都会被拒绝。

开启后，脚本正常退出、异常退出或被停止时，Guardian 会再次启动它；脚本进程被
系统回收后，前台服务恢复时也会重新启动。重试间隔依次为 5、10、20、40、60 秒，
最长保持 60 秒；实例连续健康运行 2 分钟后，下次重试会从 5 秒重新开始。
为避免两个接收器同时处理同一条命令，启用期间从 AutoX 界面再次运行同一路径脚本
会被拒绝；更换守护路径前也会先精确停止旧实例。

手机重启并首次解锁后，AutoX 会在收到 `BOOT_COMPLETED` 或
...
如需永久停止，必须关闭“启用脚本守护”开关；只停止脚本会被视为意外退出并再次启动。
