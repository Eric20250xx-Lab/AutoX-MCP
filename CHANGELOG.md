...
### Eric20250xx-Lab fork

#### 2026-08-18

* Script Guardian 增加常驻脚本自动恢复、快速启动广播兼容、主进程无障碍访问，以及主进程与 `:script` 进程之间的同路径脚本接管和去重。
* v7 调试构建版本更新为 `7.2.1-eric.7`（versionCode `727`）。
* realme `RMX3366` 已覆盖安装并在手动打开 AutoX 后通过 MCP 27 个工具、设备身份、UI 树、命令接收器 1.3.0、无障碍就绪和无业务副作用的连接测试。
* 尚未验证整机重启并首次解锁后不手动打开 AutoX 的自动恢复，以及恢复期间始终只有一个命令接收器实例；真实业务命令留到下一次正常使用时验收。Android `force-stop` 不属于自动恢复保证。

#### 2026-08-14

* MCP 服务启停改为串行且幂等，移除设置页、偏好桥和前台服务之间的重复启停，服务销毁时取消待执行任务，降低关闭时发生竞态和崩溃的风险。
* MCP Server 启动失败会返回真实结果，不再记录“启动成功”；相同配置且服务仍在运行时不再重复绑定端口。
* Android 11 及以上无障碍截图会先从硬件缓冲区复制为普通位图，并及时关闭硬件资源；截图失败改为可捕获异常并增加 5 秒超时。Android 安全窗口仍遵循系统限制，不尝试绕过。
* `ScreenshotTool` 在成功、失败和提前返回时都会释放截图对象。
* v7 调试构建版本更新为 `7.2.1-eric.2`（versionCode `722`）。
* 已通过 `:mcp:testDebugUnitTest`、`:app:compileV7DebugKotlin` 和 `:app:assembleV7Debug`；服务关闭和截图仍需在安装该构建后完成真机验收。

#### 2026-08-13

* `cancel_job` 只停止指定 MCP job，并等待对应脚本引擎退出后再返回 `CANCELED`；未知、已完成、正在取消和停止超时状态不再误报成功。
* 建立 `Eric20250xx-Lab` 社区 fork 说明，保留上游历史和许可证，并把项目链接统一指向组织仓库。
